...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="BKZ Stat Predictor Training.")
...
def main():
    ...
    checkpoint_dir = Path(
        f"stat_predictor_checkpoints/{args.dist}_dim{args.max_basis_dim}_{timestamp}"
    )
    ...
    # Initialize WandB
    wandb.init(
        project="bkz-stat-predictor-training",
    ...
    for step in range(args.steps):
        ...
        if (step + 1) % args.chkpt_interval == 0:
            checkpoint_path = checkpoint_dir / f"stat_predictor_step{step+1}.pth"
            torch.save(trainer.state_dict(), checkpoint_path)
            logging.info(f"Checkpoint saved at step {step+1}")

    # Final save
    final_path = checkpoint_dir / "stat_predictor_final.pth"
