...
class CriticNetwork(nn.Module):
    ...
    def forward(self, tensordict: TensorDict, action: torch.Tensor) -> torch.Tensor:
        ...
        combined = torch.cat([basis_features, meta_features, action_features], dim=1)
...
class DDPGAgent(nn.Module):
    def __init__(self, ddpg_config: DDPGConfig) -> None:
        ...
        # Optimizers
        self.actor_optimizer = optim.AdamW(self.actor.parameters(), lr=ddpg_config.actor_lr)
        self.critic_optimizer = optim.AdamW(self.critic.parameters(), lr=ddpg_config.critic_lr)
    ...
    def update(self, device: Union[torch.device, str]) -> None:
        ...
        batch = self.replay_buffer.sample(self.ddpg_config.batch_size).to(device)
    ...
    def train_step(self, batch: TensorDict, device: Union[torch.device, str]) -> Dict[str, float]:
        ...
        while not done and step_count < max_steps:
            ...
            self.store_transition(state, action_probs, reward, done, next_state)
